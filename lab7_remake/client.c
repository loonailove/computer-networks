...
void run_client(int sockfd) {
  ...
  */
  ...
  while(1) {
    ...
    // caz 2: primire mesaj de la server (sau un client a trimis un mesaj pe server)
    if (poll_fds[1].revents & POLLIN) {
      rc = recv_all(sockfd, &recv_packet, sizeof(struct chat_packet));
      if (rc < 0) {
        break;
      }
