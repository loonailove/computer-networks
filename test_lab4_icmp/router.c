		...
		if (ip_hdr->protocol == 1) {	// icmp
			if (icmp_hdr->type == 8) { // echo request
				...
				if (ip_hdr->ttl > 1) {
					...
					/* TTL <= 1: modify existing packet to become Time Exceeded */
					printf("DEBUG: TTL <= 1, converting to Time Exceeded\n");
					...
					/* Modify ICMP header to Time Exceeded */
					icmp_hdr->type = 11;
					icmp_hdr->code = 0;
					icmp_hdr->id = 0;
					icmp_hdr->sequence = 0;
					
					/* Update ICMP checksum for the new ICMP message */
					icmp_hdr->checksum = 0;
					int icmp_len = sizeof(struct icmphdr); // exact 8 octeti
					
					void *icmp_ptr = (void *)icmp_hdr;
